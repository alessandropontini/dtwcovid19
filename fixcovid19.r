...
dtw_align_curves <- function(data, season_target, seasons, threshold){
  ...
  for(k in 1:length(season_query_list)){
    ...
    # mat <- alignment$costMatrix
    #
    # # 4. matrix alignment algorithm -------------------------------------------
    # for (i in 1:nrow(mat)) {
    #   check = mat[i,1]
    #   for (j in 1:ncol(mat)){
    #     if(mat[i,j] <= check & mat[i,j]<threshold) {
    #       que_ali_idx = i
    #       ref_ali_idx = j
    #       check = mat[i,j]
    #     }
    ...
    # }
    #
    # x <- data_stagione_reference %>% slice(ref_ali_idx) %>% select(week_count) %>% as.numeric()
    # y <- data_stagione_query %>% slice(que_ali_idx) %>% select(week_count) %>% as.numeric()
    ...
    lag_data <- lag_data %>% add_row(season = season_query_list[k], week_lag = lag_var) %>% arrange(season)
